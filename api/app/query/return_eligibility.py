...
class ReturnEligibilityQuery:
    ...
    def for_order(self, order_id: int) -> list[ReturnableOrderItem]:
        shipped = (
            select(ShipmentItem.order_item_id.label("order_item_id"), func.sum(ShipmentItem.quantity).label("quantity"))
            .join(Shipment, Shipment.id == ShipmentItem.shipment_id)
            .where(Shipment.order_id == order_id, Shipment.status == ShipmentStatus.SHIPPED.value)
            .group_by(ShipmentItem.order_item_id)
            .subquery()
        )
        requested = (
            select(SalesReturnItem.order_item_id.label("order_item_id"), func.sum(SalesReturnItem.requested_quantity).label("quantity"))
            .join(SalesReturn, SalesReturn.id == SalesReturnItem.sales_return_id)
            .where(SalesReturn.order_id == order_id, SalesReturn.status != SalesReturnStatus.CANCELED.value)
            .group_by(SalesReturnItem.order_item_id)
            .subquery()
        )
        stmt = (
            select(SalesOrderItem.id, SalesOrderItem.product_id, func.coalesce(shipped.c.quantity, 0), func.coalesce(requested.c.quantity, 0))
        ...
        return [ReturnableOrderItem(order_item_id=row[0], product_id=row[1], shipped_quantity=int(row[2]), already_requested_quantity=int(row[3]), returnable_quantity=max(int(row[2]) - int(row[3]), 0)) for row in self.db.execute(stmt)]
