...
from app.module.business_types import OrderStatus, ShipmentStatus
...
class OperationsDashboardQuery:
    ...
    def get(self, organization_id: int) -> OperationsDashboard:
        order_stmt = select(
            func.count(case((SalesOrder.status.not_in([OrderStatus.SHIPPED.value, OrderStatus.CANCELED.value]), 1))).label("open_order_count"),
            func.count(case((SalesOrder.status == OrderStatus.ALLOCATED.value, 1))).label("ready_to_ship_count"),
            func.count(case((SalesOrder.status == OrderStatus.SHIPPED.value, 1))).label("shipped_order_count"),
            func.coalesce(func.sum(SalesOrder.total_amount), 0).label("total_order_amount"),
        ...
        stock_stmt = (
            ...
            .where(
                ...
                InventoryBalance.on_hand_quantity - InventoryBalance.reserved_quantity <= 5,
            )
        )
