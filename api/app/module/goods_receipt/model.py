...
class GoodsReceipt(Base):
    ...
    purchase_order_id: Mapped[int] = mapped_column(
        BigInteger, ForeignKey("purchase_order.id"), index=True
    )
    warehouse_id: Mapped[int] = mapped_column(
        BigInteger, ForeignKey("warehouse.id"), index=True
    )
    status: Mapped[str] = mapped_column(
        String(20), default=GoodsReceiptStatus.DRAFT.value, index=True
    )
    received_date: Mapped[date] = mapped_column(Date, nullable=False)
    supplier_reference: Mapped[str | None] = mapped_column(String(100))
    note: Mapped[str | None] = mapped_column(Text)
    received_by_account_id: Mapped[int] = mapped_column(
