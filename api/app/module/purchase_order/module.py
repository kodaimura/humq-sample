...
class PurchaseOrderModule:
    ...
    def create(self, **values) -> PurchaseOrder:
        entity = PurchaseOrder(**values)
        self.db.add(entity)
        self.db.flush()
        self.db.refresh(entity)
        return entity

    def get_by_id(self, entity_id: int) -> PurchaseOrder | None:
        return self.db.get(PurchaseOrder, entity_id)

    def get_for_update(self, entity_id: int) -> PurchaseOrder | None:
        return self.db.scalars(
            select(PurchaseOrder).where(PurchaseOrder.id == entity_id).with_for_update()
        ).first()
    ...
    def list_by_buyer(self, buyer_organization_id: int) -> list[PurchaseOrder]:
        return list(
            self.db.scalars(
                ...
                .where(PurchaseOrder.buyer_organization_id == buyer_organization_id)
    ...
    def change_status(self, entity: PurchaseOrder, status: str) -> str:
        ...
        if status == PurchaseOrderStatus.APPROVED.value:
            entity.approved_at = now
        if status == PurchaseOrderStatus.CANCELED.value:
