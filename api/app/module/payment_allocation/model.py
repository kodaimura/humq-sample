...
class PaymentAllocation(Base):
    ...
    payment_id: Mapped[int] = mapped_column(BigInteger, ForeignKey("payment.id"), index=True)
    invoice_id: Mapped[int] = mapped_column(BigInteger, ForeignKey("invoice.id"), index=True)
    amount: Mapped[Decimal] = mapped_column(Numeric(16, 2), nullable=False)
    allocated_by_account_id: Mapped[int] = mapped_column(BigInteger, ForeignKey("account.id"))
