...
class Invoice(Base):
    ...
    seller_organization_id: Mapped[int] = mapped_column(BigInteger, ForeignKey("organization.id"), index=True)
    customer_organization_id: Mapped[int] = mapped_column(BigInteger, ForeignKey("organization.id"), index=True)
    order_id: Mapped[int] = mapped_column(BigInteger, ForeignKey("sales_order.id"), index=True)
    status: Mapped[str] = mapped_column(String(30), default=InvoiceStatus.DRAFT.value, index=True)
