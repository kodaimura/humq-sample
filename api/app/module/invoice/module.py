...
class InvoiceModule:
    ...
    def create(self, **values) -> Invoice:
        entity = Invoice(**values); self.db.add(entity); self.db.flush(); self.db.refresh(entity); return entity
    def get_by_id(self, entity_id: int) -> Invoice | None: return self.db.get(Invoice, entity_id)
    def get_for_update(self, entity_id: int) -> Invoice | None:
        return self.db.scalars(select(Invoice).where(Invoice.id == entity_id).with_for_update()).first()
    def list_by_seller(self, seller_organization_id: int) -> list[Invoice]:
        return list(self.db.scalars(select(Invoice).where(Invoice.seller_organization_id == seller_organization_id).order_by(Invoice.id.desc())).all())
    def list_by_order(self, order_id: int) -> list[Invoice]:
        return list(self.db.scalars(select(Invoice).where(Invoice.order_id == order_id).order_by(Invoice.id.desc())).all())
    def set_totals(self, entity: Invoice, *, subtotal: Decimal, tax_amount: Decimal, total_amount: Decimal) -> None:
    ...
    def change_status(self, entity: Invoice, status: str) -> str:
        ...
        if status == InvoiceStatus.ISSUED.value: entity.issued_at = now
        if status == InvoiceStatus.VOID.value: entity.voided_at = now
