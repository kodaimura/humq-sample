...
class GoodsReceiptStatusHistoryModule:
    ...
    def create(self, **values) -> GoodsReceiptStatusHistory:
        entity = GoodsReceiptStatusHistory(**values)
        self.db.add(entity)
        self.db.flush()
    ...
    def list_by_receipt(self, goods_receipt_id: int) -> list[GoodsReceiptStatusHistory]:
        return list(
            self.db.scalars(
                select(GoodsReceiptStatusHistory)
                .where(GoodsReceiptStatusHistory.goods_receipt_id == goods_receipt_id)
                .order_by(GoodsReceiptStatusHistory.id)
            ).all()
        )
