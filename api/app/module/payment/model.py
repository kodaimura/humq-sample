...
class Payment(Base):
    ...
    payer_organization_id: Mapped[int] = mapped_column(BigInteger, ForeignKey("organization.id"), index=True)
    payee_organization_id: Mapped[int] = mapped_column(BigInteger, ForeignKey("organization.id"), index=True)
    status: Mapped[str] = mapped_column(String(20), default=PaymentStatus.DRAFT.value, index=True)
