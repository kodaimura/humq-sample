...
class CreateSalesReturnUsecase:
    ...
    def execute(self, input: CreateSalesReturnInput) -> SalesReturn:
        ...
        if not input.items or len({i.order_item_id for i in input.items}) != len(input.items): raise AppError(code=ErrorCode.INVALID_STATE)
        ...
        for line in input.items:
            ...
            if not eligibility or not order_item or line.quantity <= 0 or line.quantity > eligibility.returnable_quantity: raise AppError(code=ErrorCode.RETURN_QUANTITY_EXCEEDED)
            self.return_items.create(sales_return_id=entity.id, order_item_id=order_item.id, product_id=order_item.product_id, requested_quantity=line.quantity, unit_credit=order_item.unit_price)
            credit += order_item.unit_price * line.quantity
        entity.requested_credit_amount = credit; self.db.flush()
...
class ChangeSalesReturnStatusUsecase:
    ...
    def execute(self, *, account_id: int, sales_return_id: int, action: str, reason: str | None = None) -> SalesReturn:
        ...
        previous = self.returns.change_status(entity, target)
        self.history.create(sales_return_id=entity.id, from_status=previous, to_status=target, reason=reason, changed_by_account_id=account_id)
        self.outbox.enqueue(event_type=f"sales_return.{target.lower()}", aggregate_type="sales_return", aggregate_id=entity.id, payload={"sales_return_id": entity.id, "order_id": entity.order_id, "status": target})
        self.audit.record(actor_account_id=account_id, action=f"sales_return.{target.lower()}", resource_type="sales_return", resource_id=entity.id, details={"reason": reason})
        self.db.commit(); return entity
