...
class PostGoodsReceiptUsecase:
    def __init__(self, db: Session):
        ...
        self.receipt_history = GoodsReceiptStatusHistoryModule(db)
        self.order_history = PurchaseOrderStatusHistoryModule(db)
    ...
    def execute(self, *, account_id: int, goods_receipt_id: int) -> GoodsReceipt:
        ...
        if receipt.status != GoodsReceiptStatus.DRAFT.value or order.status not in {
            PurchaseOrderStatus.APPROVED.value,
            PurchaseOrderStatus.PARTIALLY_RECEIVED.value,
        }:
        ...
        for item in self.receipt_items.list_by_receipt(receipt.id):
            ...
            if item.accepted_quantity:
                ...
                self.ledger.record(
                    warehouse_id=receipt.warehouse_id,
                    product_id=item.product_id,
                    event_type=InventoryEventType.PURCHASE_RECEIPT.value,
                    on_hand_delta=item.accepted_quantity,
                    reserved_delta=0,
        ...
        previous_receipt = receipt.status
        self.receipts.post(receipt)
        self.receipt_history.create(
            goods_receipt_id=receipt.id,
            from_status=previous_receipt,
            to_status=GoodsReceiptStatus.POSTED.value,
            reason=None,
