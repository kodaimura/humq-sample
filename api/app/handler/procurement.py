...
@router.get(
    "/purchase-orders/{purchase_order_id}", response_model=PurchaseOrderDetailsResponse
)
def get_purchase_order(
    ...
    return ApiResponse.ok(
        data=PurchaseOrderDetailsResponse(
            purchase_order=PurchaseOrderResponse.model_validate(order),
...
@router.post(
    "/organizations/{organization_id}/supplier-products",
    response_model=SupplierProductResponse,
)
def configure_supplier_product(
    organization_id: int,
    ...
    entity = ConfigureSupplierProductUsecase(db).execute(
        ConfigureSupplierProductInput(
    ...
    return ApiResponse.created(
        data=SupplierProductResponse.model_validate(entity), response=response
    )


@router.post(
    "/organizations/{organization_id}/reorder-policies",
    response_model=ReorderPolicyResponse,
)
def configure_reorder_policy(
    organization_id: int,
    request: ReorderPolicyRequest,
    response: Response,
    account_id: int = Depends(get_account_id),
    db: Session = Depends(get_db),
):
    entity = ConfigureReorderPolicyUsecase(db).execute(
        ConfigureReorderPolicyInput(
            account_id=account_id,
    ...
    return ApiResponse.created(
        data=ReorderPolicyResponse.model_validate(entity), response=response
    )


@router.get(
    "/organizations/{organization_id}/reorder-recommendations",
    response_model=ReorderRecommendationsResponse,
)
def reorder_recommendations(
    ...
    return ApiResponse.ok(
        data=ReorderRecommendationsResponse(
            recommendations=[
                ReorderRecommendationResponse.model_validate(item, from_attributes=True)
                for item in items
            ]
        ),
        response=response,
...
def list_purchase_orders(
    ...
    return ApiResponse.ok(
        data=PurchaseOrdersResponse(
            purchase_orders=[
                PurchaseOrderOverviewResponse.model_validate(item, from_attributes=True)
                for item in items
            ]
        ),
        response=response,
...
@router.post(
    "/organizations/{organization_id}/purchase-orders",
    response_model=PurchaseOrderOperationResponse,
)
def create_purchase_order(
    ...
    entity = CreatePurchaseOrderUsecase(db).execute(
        CreatePurchaseOrderInput(
            account_id=account_id,
            buyer_organization_id=organization_id,
            ...
            note=request.note,
            items=[
                PurchaseOrderLineInput(**item.model_dump()) for item in request.items
            ],
        )
    )
    return ApiResponse.created(
        data=PurchaseOrderOperationResponse(
            purchase_order=PurchaseOrderResponse.model_validate(entity)
        ),
        response=response,
    )
...
def approve_purchase_order(
    ...
    entity = ApprovePurchaseOrderUsecase(db).execute(
        account_id=account_id,
...
def cancel_purchase_order(
    ...
    entity = CancelPurchaseOrderUsecase(db).execute(
        account_id=account_id,
        purchase_order_id=purchase_order_id,
        reason=request.reason,
    )
...
@router.post(
    ...
    response_model=GoodsReceiptOperationResponse,
)
def create_goods_receipt(
    ...
    entity = CreateGoodsReceiptUsecase(db).execute(
        CreateGoodsReceiptInput(
            account_id=account_id,
            purchase_order_id=purchase_order_id,
            ...
            note=request.note,
            items=[
                GoodsReceiptLineInput(**item.model_dump()) for item in request.items
            ],
        )
    )
    return ApiResponse.created(
        data=GoodsReceiptOperationResponse(
            goods_receipt=GoodsReceiptResponse.model_validate(entity)
        ),
        response=response,
    )
...
def post_goods_receipt(
    ...
    entity = PostGoodsReceiptUsecase(db).execute(
        account_id=account_id, goods_receipt_id=goods_receipt_id
    )
    return ApiResponse.ok(
        data=GoodsReceiptOperationResponse(
            goods_receipt=GoodsReceiptResponse.model_validate(entity)
