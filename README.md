...
seed には日本語の商品10件、3倉庫、顧客3社、仕入先2社と、それらに紐づく在庫・補充基準を収録しています。さらに、下書き・一部引当・引当済み・出荷済み・取消済みの受注、下書き・分納中・入荷済みの発注、移動中・入庫済みの倉庫移動、返品、請求、一部入金・全額入金までを確認できます。ログイン後、ヘッダーで `HUMQ製造株式会社` を選択してください。
